...
var Suggest = Vue.extend({
  ...
  methods: {
      ...
      $.ajax({
        ...
        success: function success(data) {
          ...
          for (m in data['sug']) {
            ...
            if (data['msg'] != "") _self.origin_msg = data['msg'];else _self.origin_msg = localStorage.getItem("msg").split(",");
          }
    ...
    selectSug: function selectSug(event) {
      //点击分词
    ...
    submitSuggestion: function submitSuggestion() {
      ...
      $.post(url, { sugs: _JSON$stringify(new_suggests) }, function (data) {
