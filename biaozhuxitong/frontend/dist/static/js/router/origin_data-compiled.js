  ...
  methods: {
    ...
    searchItem: function searchItem() {
      ...
      $.post(url, { msg: searchText, type: _self.cur_sug }, function (data) {
        ...
        _self.cur_items = _Object$assign({}, _self.cur_items);
    ...
    combineSegSug: function combineSegSug(seg, sug, index) {
      return seg + "-" + sug[index];
    },
    rewriteData: function rewriteData() {
      var _self = this;
      new_data = [];
    ...
    changePageButton: function changePageButton() {
      this.start_page = 1;
      this.select_page = 1;
      if (this.cur_sug == "所有") {
    ...
    // 切换标注类型,更新数据
    updateDataByType: function updateDataByType() {
      this.select_page = 1;
      this.total_page = Math.ceil(vutils.jsonLength(this.items_by_sug[this.cur_sug]) / this.ITEM_PER_PAGE);
      if (this.cur_sug == "所有") {
        this.total_page = Math.ceil(vutils.jsonLength(this.items) / this.ITEM_PER_PAGE);
    ...
    updatePageItems: function updatePageItems() {
      ...
      // this.updateItems(this.items_by_sug[this.cur_sug])
      if (!this.is_tmp) {
        if (this.cur_sug == "所有") this.updateItemsAll(this.items);else this.updateItems(this.items_by_sug[this.cur_sug]);
      } else {
    ...
    updateItemsAll: function updateItemsAll(item) {
      this.cur_items = [];
      //当前分词数小于每页的个数,取分词数
    ...
    save_items: function save_items() {
      ...
      var _self = this;
      $.post(url, { msg: _JSON$stringify(this.updated_sugs) }, function (data) {
        for (i in data["all_sug"]) {
          _self.items_by_sug[data["all_sug"][i]] = [];
        }
        for (i in data["items"]) {
    ...
    delete_items: function delete_items() {
      if (this.selected_items.length > 0) {
        ...
        var _self = this;
        $.post(url, { msg: _JSON$stringify(items), source: "现有文件" }, function (data) {
          for (i in data["all_sug"]) {
            _self.items_by_sug[data["all_sug"][i]] = [];
          }
          _self.items = data["items"];
          for (i in data["items"]) {
            _self.items_by_sug[data["items"][i]["sug"]].push(data["items"][i]["seg"]);
            _self.items_by_sug = _Object$assign({}, _self.items_by_sug);
          }
          _self.updateDataByType();
          vpage.updateStyle(1);
          $("#floatLayer").hide();
    ...
    checkAllTable: function checkAllTable(event) {
      obj = event.target;
      if ($(obj).prop('checked')) {
    ...
    selectItemTable: function selectItemTable(event, $thr, $tbr) {
      obj = event.target;
      $thr = $('#table thead tr');
      $tbr = $('#table tbody tr');
    ...
    //上一页选中的id
    get_seleted_items: function get_seleted_items() {
      contents = [];
    ...
    update_selected_items: function update_selected_items() {
      ...
      $.each($tbr, function (idx, obj) {
        //此时obj是上一页内容,这里只是获取checkbox,跟内容无关
        ...
        if (vutils.has_element(_self.selected_items, idx)) {
          //这个节点的内容已经选过,添加check
    ...
    update_items: function update_items() {
      if (!this.is_tmp) {
        if (this.cur_sug == "所有") items = this.items;else items = this.items_by_sug[this.cur_sug];
      } else {
    ...
    //修改标注类型
    update_option: function update_option(event) {
      obj = event.target;
...
//# sourceMappingURL=origin_data-compiled.js.map