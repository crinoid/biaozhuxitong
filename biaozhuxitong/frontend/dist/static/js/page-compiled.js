...
var vpage = new Vue({
  ...
  methods: {
    setPageCount: function setPageCount(page) {
      this.ITEM_PER_PAGE = page;
    },
    setStartPage: function setStartPage(page) {
      this.start_page = page;
    },
    getEndPage: function getEndPage(total_page) {
    ...
    updatePageButton: function updatePageButton(start_page, end_page) {
    ...
    isPrevEnable: function isPrevEnable(page_num) {
      if (page_num == 1) {
        return false;
      }
      return true;
    ...
    updateItems: function updateItems(items_by_sug, cur_sug) {
      tmp_items = [];
      //当前分词数小于每页的个数,取分词数
    ...
    updateStyle: function updateStyle(select_page) {
      if (select_page) {
